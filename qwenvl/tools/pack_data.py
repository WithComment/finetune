...
class MultimodalProcessor:
  ...
  def process_image(self, image_file):
    ...
    if not os.path.exists(image_path):
      ...
      return 0
    processor = self._configure_img_processor(
        self.data_args.max_pixels, self.data_args.min_pixels)
  ...
  def process_video(self, video_file):
    ...
    processor = self._configure_vid_processor(
    ...
    with av.open(BytesIO(vid_data)) as container:
      ...
      target_frames = min(max(
          num_frames_to_sample, self.data_args.video_min_frames), self.data_args.video_max_frames)
    ...
    visual_processed = processor.preprocess(
        images=None, videos=video_frames_numpy, return_tensors='pt')
...
def calculate_tokens(conversation, processor, tokenizer):
  ...
  for message in conversation['conversations']:
    ...
    encode_id = tokenizer.apply_chat_template(
        conv, return_tensors='pt', add_generation_prompt=False)[0]
    total_tokens += len(encode_id)
  if 'image' in conversation:
    images = conversation['image'] if isinstance(
        conversation['image'], list) else [conversation['image']]
    for image_file in images:
      total_tokens += processor.process_image(image_file)
  elif 'video' in conversation:
    videos = conversation['video'] if isinstance(
        conversation['video'], list) else [conversation['video']]
...
if __name__ == '__main__':
  ...
  argparser.add_argument('--model_path', type=str,
                         default='Qwen/Qwen2.5-VL-3B-Instruct', help='Path to the model')
  argparser.add_argument('--data_path', type=str,
                         default='/projects/cft_vlm/openbiomedvid/vid_processed')
  argparser.add_argument('--pack_length', type=int,
                         default=8192, help='Length of the packed data')
  argparser.add_argument('--batch_size', type=int,
                         default=256, help='Batch size for processing')
  ...
  processor = MultimodalProcessor(
      data_args, img_processor, vid_processor, device='cpu')
  ...
  for dataset_name, config in datasets.items():
    ...
    print(
        f'Image configuration: max_pixels={data_args.max_pixels}, min_pixels={data_args.min_pixels}')
    print(
    ...
    if os.path.exists(count_file_path):
      print(
          f"Found pre - calculated token counts, loading data from {count_file_path}.")
      ...
      with concurrent.futures.ThreadPoolExecutor() as executor:
        data_with_tokens = list(tqdm(executor.map(calculate_and_update, data), total=len(
            data), desc=f"Processing {dataset_name} data"))
